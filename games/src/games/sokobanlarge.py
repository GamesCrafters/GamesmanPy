...
class SokobanLarge(Game):
    ...
    def __init__(self, variant_id: str):
        ...
        """
        if variant_id not in SokobanLarge.variants:
    ...
    def primitive(self, position: str) -> Optional[Value]:
        ...
        while idx != -1:
            ...
            blocks = [
                (idx - 1, idx - col, idx - col - 1),  
                (idx + 1, idx - col, idx - col + 1),  
                (idx - 1, idx + col, idx + col - 1),  
                (idx + 1, idx + col, idx + col + 1)  
            ]
            
            if any(all(position[i] in blocker for i in block) for block in blocks):
                return Value.Loss
