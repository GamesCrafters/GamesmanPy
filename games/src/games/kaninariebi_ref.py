ßßfrom models import Game, Value, StringMode
from typing import Optional


class KaniNariEbi(Game):
    """Kani Nari Ebi -- standard same-type Bond rules.

    Board encoding (row-major, 5x5):
        0 = empty
        1 = Player 1 Crab
        2 = Player 1 Shrimp
        3 = Player 2 Crab
    ...
    Player 1 starts on the left home column.
    Player 2 starts on the right home column.

    A solver move encodes the COMPLETE turn:
        main movement
        + optional sea-current direction
        + optional promotion
        + optional Bond bonus movement
        + optional Bond sea-current direction
        + optional Bond promotion

    Captures are deterministic and therefore are not stored inside the move.
    """

    id = "kaninariebi"
    variants = ["standard", "mixed"]
    n_players = 2

    # Reversible movement means cycles are possible before captures/promotions.
    cyclic = True
    ...
    def do_move(self, position: int, move: int) -> int:
        ...
        if bond_part is not None and not self._wins_immediately(
            new_board, player
        ):
