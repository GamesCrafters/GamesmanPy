...
class TUI:
    ...
    def play(self, game: Game):
        ...
        while game.primitive(curr_pos) is None:
            print("\n" + "=" * 40)
            print("Current position:")
            print(game.to_string(curr_pos, StringMode.TUI))
            ...
            for i, move in enumerate(moves, start=1):
                move_str = game.move_to_string(move, StringMode.TUI)
                print(f"{i}. {move_str}")

            while True:
                user_input = input("\nSelect move number (q to quit): ").strip()
